...
namespace GameDevJourney
{
	public class ObjectPool : SingletonMonoBehaviour<ObjectPool>
	{
		...
		public virtual void CancelDelayedDespawn (DelayedDespawn delayedDespawn)
		{
			...
			{
				delayedDespawns.RemoveAt(indexOfDelayedDespawn);
				if (delayedDespawns.Count == 0 && rangedDespawns.Count == 0)
					enabled = false;
			}
		}
		...
		public virtual void CancelRangedDespawn (RangedDespawn rangedDespawn)
		{
			...
			{
				rangedDespawns.RemoveAt(indexOfRangedDespawn);
				if (rangedDespawns.Count == 0 && delayedDespawns.Count == 0)
					enabled = false;
			}
		}
		...
		public virtual T SpawnComponent<T> (int prefabIndex, Vector3 position = new Vector3(), Quaternion rotation = new Quaternion(), Transform parent = null)
		{
			...
			return SpawnComponent<T>(prefabIndex, position, rotation, parent);
		}
